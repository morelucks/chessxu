...
- Supported fee currencies: **USDm · USDC · USDT · EURm · BRLm**

### 💰 Daily Access Payments
- **USDm micro-payment** (0.1 USDm) unlocks 24-hour match access.
...
| `src/hooks/useMiniPayAccess.ts` | Daily USDm access purchase + balance tracking |
...
6. Purchase daily access with USDm (gas paid in USDm — no CELO needed).
...
- On-chain `Transfer` event verification for all USDm payments.
