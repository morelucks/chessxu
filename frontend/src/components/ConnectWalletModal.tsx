...
export const ConnectWalletModal: React.FC = () => {
  ...
  const handleSelectChain = async (chain: "stacks" | "celo") => {
    setConnectModalOpen(false);
    // Trigger connection for the selected chain
    await connect({ chain });
  };
    ...
    <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-[100] backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-slate-950 border border-slate-800/80 rounded-2xl p-6 w-full max-w-md shadow-2xl relative animate-in zoom-in-95 duration-200 flex flex-col gap-5">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400">Choose a blockchain network to play</p>
            </div>
          </div>
          <button
        ...
        <div className="flex items-center gap-2 p-3 bg-white/5 rounded-xl border border-white/5 text-[10px] text-slate-400 leading-snug">
          ...
          <span>Need access? Celo supports gas-sponsored matches. Stacks features custom wager sizes.</span>
        </div>
      </div>
    </div>
  );
